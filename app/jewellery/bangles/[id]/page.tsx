...
import { X, ShoppingBag, Gem, Scale, Scan, PackageCheck } from 'lucide-react';
// 1. Change import from EarringProduct to BangleProduct
import { BangleProduct } from "@/types/bangles"; 
// 1. Change import from earringList to bangleList (assuming the path is similar)
import { bangleList } from '@/data/jewellery/bangleProducts'; 


// 3. Rename component
export default function BangleDetailPage() {
  // 2. Update state type to BangleProduct
  const [product, setProduct] = useState<BangleProduct | null>(null);
  ...
  useEffect(() => {
    ...
    if (id && bangleList[id]) { // Use bangleList
      setProduct(bangleList[id]);
    }
  }, []);

  if (!product) {
    // 4. Update loading text
    return <div className="flex items-center justify-center text-black h-screen">Loading Bangle Details...</div>;
  }

  // Not strictly needed for 'bangles' category, but kept for consistency if needed elsewhere
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="w-full max-w-2xl mt-8 md:mt-12 text-center">
            ...
            <div className="flex justify-center gap-6 md:gap-8 border-y border-gray-800 py-4 mb-8">
              ...
              {/* 5. Replace 'Type' with 'Size/Dimensions' */}
              <div className="flex items-center gap-2 text-gray-700">
                <Scan className="w-5 h-5 text-amber-600" /> {/* Use Scan icon for size */}
                <span className="font-medium">{product.size} mm</span> {/* Display size */}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="border border-gray-800 rounded-lg p-4 text-left">
                  <div className="space-y-3">
                      ...
                      {/* 5. Replace 'Type' with 'Size/Dimensions' in the detail box */}
                      <div className="flex justify-between text-gray-800">
                        <span className="text-gray-500">Size (A x B)</span>
                        <span className="font-medium">{product.size} mm</span>
                      </div>
                  </div>
              </div>
            </div>
      ...
      <div className="poppins fixed bottom-0 left-0 text-black z-999 right-0 bg-amber-50 shadow-[0_-2px_10px_rgba(0,0,0,0.1)] p-3 ">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
              <div className="hidden md:flex items-center gap-6">
                  ...
                  {/* 5. Replace 'Type' with 'Size' in the footer */}
                  <div className="text-sm">Size: <span className="font-semibold">{product.size} mm</span></div>
